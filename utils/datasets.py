...
class LoadImages:  # for inference
    def __init__(self, path, img_size=640, stride=32, frame_skip=1):
        """
        FIXED: Changed to use frame_skip parameter with default=1 to process every frame
        frame_skip=1 means process every frame, frame_skip=2 means skip 1 frame (process every 2nd), etc.
        """
        ...
        # CHANGED: Use frame_skip parameter directly, default=1 for every frame
        self.frame_skip = max(1, int(frame_skip))  # Ensure minimum skip of 1
        
        if any(videos):
            self.new_video(videos[0])  # new video
            # REMOVED: Old automatic frame skipping based on FPS
            # Now uses the frame_skip parameter directly
        else:
            self.cap = None
    ...
    def __next__(self):
        ...
        if self.video_flag[self.count]:
            # CHANGED: Modified video reading logic with frame_skip
            self.mode = 'video'
            ret_val, img0 = self.cap.read()
            
            # CHANGED: Skip frames based on frame_skip parameter
            ...
            if not ret_val:
                ...
                if self.count == self.nf:  # last video
                    ...
                    self.new_video(path)
                    ret_val, img0 = self.cap.read()
            self.frame += self.frame_skip  # CHANGED: Increment by frame_skip amount
...
class LoadStreams:  # multiple IP or RTSP cameras
    def __init__(self, sources='streams.txt', img_size=640, stride=32, max_frames=0):
        ...
        for i, s in enumerate(sources):
            ...
            if 'youtube.com/' in url or 'youtu.be/' in url:  # if source is YouTube video
                # Replace pafy/youtube-dl with yt-dlp
                check_requirements(('yt-dlp',))
                import yt_dlp
                
                ydl_opts = {
                    'format': 'best[ext=mp4]/best',  # Get best mp4 format or best available
                    'quiet': True,
                    'no_warnings': True,
                    'extract_flat': False,
                ...
                try:
                    with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                        ...
                        # Get direct video URL - equivalent to pafy.new(url).getbest(preftype="mp4").url
                        url = info['url']
            ...
            self.fps = fps if fps and fps % 100 else 30  # Default to 30 if invalid

            ret, self.imgs[i] = cap.read()  # guarantee first frame
            assert ret, f'Failed to read first frame from {s}'
        ...
        s = np.stack([letterbox(x, self.img_size, stride=self.stride, auto=False)[0].shape for x in self.imgs], 0)
    ...
    def __next__(self):
        ...
        # Letterbox - ensure auto=False for consistent shapes
        img = [letterbox(x, self.img_size, auto=False, stride=self.stride)[0] for x in img0]
